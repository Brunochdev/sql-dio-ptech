...
- #### Este README fornece informações sobre a criação e estruturação de um banco de dados para um cenário de e-commerce. O banco de dados é criado usando comandos SQL e é composto por várias tabelas que representam entidades relacionadas ao funcionamento de um negócio de comércio eletrônico. ## 
#
...
O banco de dados é composto por várias tabelas, cada uma representando uma entidade ou aspecto do e-commerce.  
Abaixo, você encontrará uma descrição das principais tabelas e seus campos:

    Tabela cliente  
idCliente: Identificador único do cliente (chave primária).  
primNome, meioNome, ultNome: Nome completo do cliente.  
documento: CPF ou CNPJ do cliente.  
endereco: Endereço de entrega do cliente.  
tipoDocumento: Indica se o documento é um CPF ou CNPJ.  
#
    Tabela produto  
idProduto: Identificador único do produto (chave primária).  
prodNome: Nome do produto.  
classEtaria: Indica se o produto é para crianças (true/false).  
categoria: Categoria do produto (Eletrônico, Vestuário, Alimento, Saúde, Móveis, Brinquedo).  
avaliacao: Avaliação média do produto.  
dimEmbalagem: Dimensões da embalagem do produto.  
#
    Tabela pagamento
idCliente: ID do cliente associado ao pagamento.  
idPagamento: ID do pagamento (chave primária).  
tipoPagamento: Tipo de pagamento (Dinheiro, Cartão, Boleto, Pix).  
limite: Limite de pagamento.  
idFormaPagamento: ID da forma de pagamento associada ao pagamento.  
#
    Tabela pedido
idPedido: Identificador único do pedido (chave primária).  
idPedidoCliente: ID do cliente associado ao pedido.  
pedido: Status do pedido (Confirmado, Cancelado, Processando).  
desPedido: Descrição do pedido.  
frete: Valor do frete.  
pagamento: Indica se o pagamento foi efetuado (true/false).  
codigoRastreio: Código de rastreamento do pedido.  
#
    Tabela estoque
idProdEstoque: Identificador único do produto no estoque (chave primária).  
localEstoque: Localização do estoque.  
...
#
    Tabela vendedor
idVendedor: Identificador único do vendedor (chave primária).  
razaoSocial: Razão social do vendedor.  
fantasia: Nome fantasia do vendedor.  
CNPJ: CNPJ do vendedor.  
CPF: CPF do vendedor.  
localizacao: Localização do vendedor.  
contato: Informações de contato do vendedor.  
#
    Tabelas de Relacionamento
#    
    Tabela vendedorProduto
idProdutoV: ID do vendedor associado ao produto (chave primária).  
idProduto: ID do produto associado ao vendedor (chave primária).  
produtoQuant: Quantidade do produto fornecida pelo vendedor.
#  
    Tabela pedidoProduto
idPoProduto: ID do produto associado ao pedido (chave primária).  
idPoPedido: ID do pedido associado ao produto (chave primária).  
pedidoQuant: Quantidade do produto no pedido.  
pedidoStatus: Status do produto no pedido (Disponível, Sem Estoque). 
#
    Tabela localEstoque
idLocProduto: ID do produto associado à localização (chave primária).  
idLocEstoque: ID da localização do estoque associada ao produto (chave primária).  
location: Nome ou descrição da localização do estoque.  
#
    Tabela produtoFornecedor
idPsFornecedor: ID do fornecedor associado ao produto (chave primária).  
idPsProduto: ID do produto associado ao fornecedor (chave primária). 
quantidade: Quantidade de produtos fornecidos pelo fornecedor. 
#
    Tabela vendedorFornecedor
idVendedor: ID do vendedor associado ao fornecedor (chave primária).  
idFornecedor: ID do fornecedor associado ao vendedor (chave primária).

 ## Criação e Preenchimento do Banco de Dados ##
O banco de dados pode ser criado utilizando os comandos SQL fornecidos no início deste README.
#
...
-- cria um banco de dados com o nome de ecommerce   

use ecommerce;  
-- para iniciar o banco de dados com o nome de ecommerce  
